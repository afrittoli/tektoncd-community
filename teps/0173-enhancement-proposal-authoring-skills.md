...
| No standardized way exists yet to make a locally-installed skill available to an agent running in a sandboxed or cloud harness | Documented as a known limitation for v1 (see [Installing the Skills](#installing-the-skills)); such agents fall back to `AGENTS.md` prose with weaker triggering; revisit once the MCP [Skills Over MCP working group](https://modelcontextprotocol.io/community/working-groups/skills-over-mcp)'s extension (currently [SEP-2640](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2640), in review) is implemented by clients (see References and [Future Enhancements](#future-enhancements)) |
...
- **Distribution standard**: the MCP [Skills Over MCP working
  group](https://modelcontextprotocol.io/community/working-groups/skills-over-mcp)
  is defining exactly this kind of cross-harness distribution, via a
  `skills/list` and `skills/get` extension to MCP (currently
  [SEP-2640](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2640),
  in review; see also [PR
  #110](https://github.com/modelcontextprotocol/experimental-ext-skills/pull/110)
  for the current v1 scope). Its content model is already aligned with
  [agentskills.io](https://agentskills.io) `SKILL.md`/`references/`
  layout, so `.agents/skills/` would not need to change shape to be
  served this way, only gain a serving layer. If the SEP lands and gains
  client support, revisit serving `tektoncd/community/.agents/skills/`
  over MCP to reach agents running in sandboxed or cloud environments
  that can't read a contributor's local install.

  The natural home for that server is **not** `tektoncd/mcp-server`:
  that project is cluster/runtime-facing (it manages live Pipelines,
  TaskRuns, and PipelineRuns against a kubeconfig, plus Artifact Hub
  discovery), a different audience and trust boundary than a
  contributor authoring a TEP. A dedicated server, hosted in
  `tektoncd/community` alongside the skills it serves, is a better fit:
  since v1 of the extension supports pure static-content serving
  (per-file digest manifests, no stateful backend), it can be a thin
  process that only implements `skills/list`/`skills/get` (and
  optionally `resources/directory/read` for the `references/`
  subdirectories) directly against the files already in
  `.agents/skills/`, with no credentials to manage and no state beyond
  the repository content itself — the same "no service to host or
  credential to manage" reasoning that ruled out an MCP server for TEP
  numbering (see [Alternatives](#alternatives)) still holds here, except
  a static skills-serving shim is small enough that it may not violate
  it.
...
- [experimental-ext-skills PR #110: v1 scope decisions](https://github.com/modelcontextprotocol/experimental-ext-skills/pull/110)
- [tektoncd/mcp-server](https://github.com/tektoncd/mcp-server)
